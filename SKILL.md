...
| **native Sonnet subagent** (Agent tool, model sonnet, worktree isolation) | agentic fallback | when codex is usage-limited: same brief, same constraints, same review — the work keeps moving. Spends Claude usage; codex resumes as primary when its limit resets. |
